...
positiveFixtures :: [(String, String, SynExpr, [(String, InternedName)])]
positiveFixtures =
    ...
    , ("Variable", "x", SVar in0 dbi0, [("x", in0)])
    , ("Variable with index", "x@1", SVar in0 dbi1, [("x", in0)])
    , ("Application of two variables", "f x", SApp (SVar in0 dbi0) (SVar in1 dbi0), [("f", in0), ("x", in1)])
    , ("Application of three variables", "f x y", SApp (SApp (SVar in0 dbi0) (SVar in1 dbi0)) (SVar in2 dbi0), [("f", in0), ("x", in1), ("y", in2)])
    , ("Lambda", "\\(x : Natural) -> x", SLam in0 builtinNatural (SVar in0 dbi0), [("x", in0)])
    , ("Lambda with different names", "\\(a : Type) -> \\(b : a) -> b", SLam in0 builtinType (SLam in1 (SVar in0 dbi0) (SVar in1 dbi0)), [("a", in0), ("b", in1)])
    , ("Forall", "forall (x : Natural) -> x", SForall in0 builtinNatural (SVar in0 dbi0), [("x", in0)])
    , ("Forall with Unicode ∀", "∀(x : Natural) -> x", SForall in0 builtinNatural (SVar in0 dbi0), [("x", in0)])
    , ("Let single binding", "let x = 123 in x", SLet in0 (SNatLit 123) (SVar in0 dbi0), [("x", in0)])
    , ("Let multiple bindings", "let x = 1 let y = 2 in x + y", SLet in0 (SNatLit 1) (SLet in1 (SNatLit 2) (SApp (SApp builtinPlus (SVar in0 dbi0)) (SVar in1 dbi0))), [("x", in0), ("y", in1)])
    ...
    , ("Type annotation", "x : Natural", STypeAnn (SVar in0 dbi0) builtinNatural, [("x", in0)])
    , ("Parenthesized expression", "(123)", SNatLit 123, [])
    , ("Parenthesized application", "(f x)", SApp (SVar in0 dbi0) (SVar in1 dbi0), [("f", in0), ("x", in1)])
    , ("Complex expression: lambda with application", "\\(f : Natural -> Natural) -> f 42", SLam in0 (SForall in1 builtinNatural builtinNatural) (SApp (SVar in0 dbi0) (SNatLit 42)), [("f", in0), ("_", in1)])
    , ("Arrow type shorthand", "Natural -> Type", SForall in0 builtinNatural builtinType, [("_", in0)])
    , ("Arrow in annotation", "id : forall (a : Type) -> a -> a", STypeAnn (SVar in0 dbi0) (SForall in1 builtinType (SForall in2 (SVar in1 dbi0) (SVar in1 dbi0))), [("id", in0), ("a", in1), ("_", in2)])
    , ("Expression with line comment", "x -- this is a comment\n + y", SApp (SApp builtinPlus (SVar in0 dbi0)) (SVar in1 dbi0), [("x", in0), ("y", in1)])
    , ("Identifier with --", "x--", SVar in0 dbi0, [("x--", in0)])
    , ("Let with identifier containing --", "let x-- = 123 in x--", SLet in0 (SNatLit 123) (SVar in0 dbi0), [("x--", in0)])
    , ("Forall with identifier containing --", "forall (let-- : Natural) -> let--", SForall in0 builtinNatural (SVar in0 dbi0), [("let--", in0)])
    ]
  where
    in0 = InternedName 0
    in1 = InternedName 1
...
spec = describe "Parser" $ do
    describe "Positive tests" $
        forM_ positiveFixtures $ \(desc, input, expected, names) ->
            it desc $ do
                let (result, names') = parseExprFromString input
                result `shouldBe` Right expected
                names' `shouldBe` Map.fromList names
    describe "Negative tests" $
        forM_ negativeFixtures $ \(desc, input) ->
            it desc $ parseExprFromString input `shouldSatisfy` isLeft . fst
  where
    isLeft (Left _) = True
    isLeft _ = False
