...
spec = do
    describe "type checker positive cases" $ do
        ...
        it "infers builtin type for Natural/subtract" $
            inferTypeLazy (EBuiltin NaturalSubtract)
                `shouldBe` Right (EForall "_" (EBuiltin Natural) (EForall "_" (EBuiltin Natural) (EBuiltin Natural)))

        it "infers builtin type for Natural/fold" $
            inferTypeLazy (EBuiltin NaturalFold)
                `shouldBe` Right
                    ( EForall
                        "_"
                        (EBuiltin Natural)
        ...
        it "accepts annotation that uses Kind as a type expression" $
            inferTypeLazy (EAnnot (EBuiltin Type) (EBuiltin Kind)) `shouldBe` Right (EBuiltin Kind)

        it "resolves let-bound type aliases during application" $
            inferTypeLazy
                ( ELet
                    "N"
                    (EBuiltin Natural)
                    ( ELet
                        "idN"
                        (ELam "x" (EVar (Var "N" 0)) (EVar (Var "x" 0)))
                        (EApp (EVar (Var "idN" 0)) (ENatural 42))
                    )
                )
                `shouldBe` Right (EBuiltin Natural)
    ...
    describe "type checker negative cases" $ do
        ...
        it "rejects term expressions used as types" $
            inferTypeLazy (ELam "x" (ENatural 0) (EVar (Var "x" 0)))
                `shouldBe` Left (InvalidTypeExpression (EBuiltin Natural))
        ...
        it "infers Natural for both arithmetic operators" $ do
            inferTypeLazy (EBinOp Plus (ENatural 1) (ENatural 2)) `shouldBe` Right (EBuiltin Natural)
            inferTypeLazy (EBinOp Times (ENatural 3) (ENatural 4)) `shouldBe` Right (EBuiltin Natural)
