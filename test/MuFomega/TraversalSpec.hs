...
spec = do
  ...
  describe "fold traversal order" $ do
    it "visits lazy nodes pre-order" $ do
      foldExprLazy (\acc e -> acc <> [lazyTag e]) [] sampleLazy `shouldBe`
        [ "let"
        , "nat"
        , "lam"
        , "builtin"
    ...
    it "visits strict nodes pre-order" $ do
      foldExprStrict (\acc e -> acc <> [strictTag e]) [] sampleStrict `shouldBe`
        [ "let"
        , "nat"
        , "lam"
        , "builtin"
  ...
  describe "binder-aware traversal" $ do
    it "tracks depth for lazy lambda/forall/let bodies only" $ do
      collectLazyDepths binderSampleLazy `shouldBe`
        [ ("x", 0)
        , ("x", 1)
        , ("x", 2)
        , ("x", 3)
    ...
    it "tracks depth for strict lambda/forall/let bodies only" $ do
      collectStrictDepths binderSampleStrict `shouldBe`
        [ ("x", 0)
        , ("x", 1)
        , ("x", 2)
        , ("x", 3)
